...
const CategoriesPage = () => {
    ...
    if (categories?.data) {
      setAllCategories((prevCategories) => {
        const newCategories = categories.data.filter(
          (newCategory) => !prevCategories.some((category) => category.id === newCategory.id)
        )
  ...
  return (
    <div className="pt-nav">
      <SEOHead title="Catégories de site d'annonces au Maroc - MarocAdsZone" description="Explorez toutes les catégories d'annonces gratuites au Maroc : immobilier, auto, emploi, services, multimédia, et plus sur MarocAdsZone." path="/categories" />
      <div className="app-container page-py">
        <PageHeader>
          <h1 className="title-h1">
            Explorez toutes les catégories d'annonces gratuites
          </h1>
          <p className="text-base text-gray-400">
            {t("categories.see_all_categories")}
          </p>
        </PageHeader>
        ...
        <div className="mb-6">
          ...
          className="section-py grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5"
        >          {isError && (
          ...
          {!isLoading && !isError && filteredCategories.length === 0 && allCategories.length > 0 && (
        ...
        </section>        {categories?.last_page && categories.last_page > 1 && categories.last_page !== page && (
