...
const StepsCalendar = ({ today, selectedDay, setSelectedDay, rtl, lang, availableDays }: StepsCalendarProps) => {
  ...
  return (
    <div>
      <div className="w-full bg-white z-10 sm:p-4">
        <div className="flex items-center">
          ...
          <button
            ...
            className={`-my-1.5 ${rtl && 'rotate-180'} border border-gray-300 hover:bg-gray-100 rounded-full flex flex-none items-center justify-center p-1.5 text-primary-blue-all-800 hover:text-gray-500 ${currentMonth === todayMonth && 'opacity-0'}`}
          >
          ...
          <button
            onClick={nextMonth}
            type="button"
            className={`-my-1.5  ${rtl && 'rotate-180'} -mr-1.5 ml-2 border border-gray-300 hover:bg-gray-100 rounded-full flex flex-none items-center justify-center p-1.5 text-primary-blue-all-800 hover:text-gray-500`}
          >
            <span className="sr-only">Next month</span>
            <FaArrowLeft  className="w-5 h-5 transform rotate-180" aria-hidden="true" />
          </button>
        </div>
        <div className="grid grid-cols-7 mt-10 text-sm leading-6 text-center text-primary-blue-all-800 font-semibold">
          <div>
            {
              lang === 'ar' ? 'الإثن' : lang === 'fr' ? 'lun' : 'Mo'
            }
          </div>
          <div>
            {
              lang === 'ar' ? 'الثل' : lang === 'fr' ? 'mar' : 'Tu'
            }
          </div>
          <div>
            {
              lang === 'ar' ? 'الأرب' : lang === 'fr' ? 'mer' : 'We'
            }
          </div>
          <div>
            {
              lang === 'ar' ? 'الخ' : lang === 'fr' ? 'jeu' : 'Th'
            }
          </div>
          <div>
            {
              lang === 'ar' ? 'الج' : lang === 'fr' ? 'ven' : 'Fr'
            }
          </div>
          <div>
            {
              lang === 'ar' ? 'الس' : lang === 'fr' ? 'sam' : 'Sa'
            }
          </div>
          <div>
            {
              lang === 'ar' ? 'الأح' : lang === 'fr' ? 'dim' : 'Su'
            }
          </div>
        </div>
